...
運用: 本リポジトリ（`C:\Users\ha.takaku\Desktop\Project\Beacon-winui`）を作業ディレクトリとして `codex exec --model gpt-5.6-sol "<プロンプト本文>"`。
...
## リポジトリ配置（重要）
- 本リポジトリ（Crowlxy/Beacon・作業対象）: C:\Users\ha.takaku\Desktop\Project\Beacon-winui
- 調査対象のBeacon-old（Crowlxy/Beacon-old・読み取り専用）: C:\Users\ha.takaku\Desktop\Project\Beacon
  ※フォルダ名は「Beacon」だが中身は旧WPF版（Beacon-old）。**このフォルダ内のファイルを一切変更しない**
...
## リポジトリ配置
- 本リポジトリ（作業対象）: C:\Users\ha.takaku\Desktop\Project\Beacon-winui
- Beacon-old（参照のみ・変更禁止）: C:\Users\ha.takaku\Desktop\Project\Beacon
