...
> **実装値の記録（2026-07-23）**: WinUI実測調整により、実装（DesignTokens.xaml）はウィンドウ幅560 / 検索バー高56 / 検索文字18px / 最大結果7件。R4のUI実装時からこの値であり、Gate B（R4.3）・Gate C（統合R5）のユーザー目視承認はこの実装値に対して行われた。上表は参照画像由来の初期値として保持する。
